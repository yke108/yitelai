...
class PointGiftService{
	const StatusNone = 0; //未审核
	const StatusPass = 1; //审核通过
	const StatusNoPass = 2; //审核不通过
	...
	public function findInfo($map){
		$info=$this->PointGiftDao()->findRecord($map);
		if(!empty($info)){
			$info['goods_gallery']=$info['gallery']!=''?explode('#',trim($info['gallery'],'#')):array();
		}
		return $info;
	...
	private function PointGiftDao(){
		return D('Common/Point/PointGift');
	}
	...
	public function getOrderInfo($id){
		...
		$user_info = $this->userDao()->getRecord($info['user_id']);
		$info['avatar'] = $user_info['user_img'] ? picurl($user_info['user_img'], 'b90') : $user_info['headimgurl'];
	...
	public function orderCreateOrModify($params){
		...
		if(empty($address_info)){
			throw new \Exception('请设置默认地址',10000);
		}
	...
	public function orderCheck($params) {
		if(empty($params['id'])) throw new \Exception('缺少参数');
		if (!in_array($params['status'], array(self::StatusPass, self::StatusNoPass))) throw new \Exception('审核状态不正确');
		$map = array('id'=>$params['id'],'status'=>self::StatusNone);
		$order_info = $this->pointGiftOrderDao()->findRecord($map);
		if(empty($order_info)) throw new \Exception('兑换订单不存在');
	...
	public function orderPagerList($params){
		...
		if ($params['keyword']) {
			...
			$order_list = $this->pointGiftOrderDao()->getFieldRecord($where, 'id, user_id');
			if (empty($order_list)) {
				return array();
			}
			...
			$where = array(
					...
					'gift_name'=>array('like', '%'.trim($params['keyword']).'%'),
			);
			$order_goods_list = $this->pointGiftOrderGoodsDao()->getFieldRecord($where, 'id, order_id, gift_name');
		...
		if ($params['gift_name']) {
			$where = array(
					'gift_name'=>array('like', '%'.trim($params['gift_name']).'%'),
			);
			$order_goods_list = $this->pointGiftOrderGoodsDao()->getFieldRecord($where, 'id, order_id, gift_name');
			if (empty($order_goods_list)) {
				return array();
		...
		if ($params['order_sn']) {
			$map['order_sn'] = array('like', '%'.trim($params['order_sn']).'%');
		}
		...
		if($count > 0){
			...
			if(!empty($user_id)){
				$user_list=$this->userDao()->getFieldRecord(array('user_id'=>array('in',$user_id)),'user_id,nick_name,real_name,user_img,headimgurl');
			}
			foreach($list as $key=>$val){
				$list[$key]['gift_name']=$gift_list[$val['gift_id']]['name'];
				$list[$key]['nick_name']=$user_list[$val['user_id']]['nick_name'];
				$list[$key]['avatar']=$user_list[$val['user_id']]['user_img'] ? picurl($user_list[$val['user_id']]['user_img'], 'b90') : $user_list[$val['user_id']]['headimgurl'];
				$list[$key]['user_name']=$user_list[$val['user_id']]['real_name'] ? $user_list[$val['user_id']]['real_name'] : $user_list[$val['user_id']]['nick_name'];
				$list[$key]['order_goods']=$gift_list[$val['gift_id']];
				//状态
				$list[$key]['status_label'] = Status::$giftStatusList[$val['status']];
			}
		}
	...
	private function pointGiftOrderDao(){
		return D('Common/Point/PointGiftOrder');
	}
	
	private function pointGiftOrderGoodsDao(){
		return D('Common/Point/PointGiftOrderGoods');
	}
