...
class ActivityCatService{
	public function getCat($id){
		if ($id < 1) return false;
		return $this->activityCatDao()->getRecord($id);
	}
	
	public function getCatInfo($map){
		return $this->activityCatDao()->findRecord($map);
	}
	...
	public function catCreateOrModify($params){
		...
		$activityCatDao = $this->activityCatDao();
		if (!$activityCatDao->create($data)){
			throw new \Exception($activityCatDao->getError());
		}
		if ($params['cat_id'] > 0){
			$result = $activityCatDao->saveRecord($data);
			if ($result === false){
				throw new \Exception('修改失败');
			}
		} else {
			$result = $activityCatDao->addRecord($data);
			if ($result < 1){
	...
	public function catDelete($id){
		...
		$result = $this->activityCatDao()->delRecord($id);
	...
	public function catPagerList($params){
		...
		$activityCatDao = $this->activityCatDao();
		$count = $activityCatDao->searchRecordsCount($map);
		$list = array();
		if($count > 0){
			$orderby = empty($params['orderby']) ? 'sort_order desc' : $params['orderby'];
			$list = $activityCatDao->searchRecords($map, $orderby, $params['page'], $params['pagesize']);
		}
		return array(
				'list'=>$list,
				'count'=>$count,
	...
	public function catAllList($map, $orderby){
		return $this->activityCatDao()->searchAllRecords($map, $orderby);
	}
	
	public function catTreeList($map, $orderby){
		$list = $this->activityCatDao()->searchAllRecords($map, $orderby);
	...
	public function catChilds($cat_id){
		$use_category = false;
		$cat_list = $this->activityCatDao()->getField('cat_id, parent_id');
		$cats = array();
	...
	private function outputForList($list){
		if (!empty($list)) {
			...
			$cats = $this->activityCatDao()->allRecordsField($map);
			foreach ($list as $k => $v) {
	...
	private function activityCatDao(){
		//返回model
		return D('Common/ActivityInfo/Cat');
	}
}//end HelpService!甜品