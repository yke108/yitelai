...
class GoodsSceneModel extends RelationModel {
	// tableName属性来改变默认的规则
    protected $tableName = 'goods_scene';
    protected $pk = 'scene_id';
    
    protected $_validate = array(
    		array('name','require','名称不能为空'), //默认情况下用正则进行验证
    		array('name','','名称已存在',0,'unique',3), // 验证name字段是否唯一
	...
	public function delRecords($ids){
		return $this->where(array('scene_id'=>array('in',$ids)))->delete();
	}
    ...
    public function searchAllRecords($map, $orderBy){
    	return $this->where($map)->order($orderBy)->getField('scene_id, name');
    }
