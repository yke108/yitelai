...
class JPush {
    ...
    public function pushByAlias($info){
    	$extral = array();
    	foreach ($info as $ko => $vo){
    		if(substr($ko, 0, 2) != 't_') continue;
    		$extral['type'] = $ko;
    		$extral['value'] = $vo;
    	}
    	$is_production = $info['debug'] ? false : true;
		$result = $this->push()
		    ->setPlatform(array('ios', 'android'))
		    ->addAlias($info['alias'])
		    ->addIosNotification($info['message'], 'default', null, null, null, $extral)
		    ->addAndroidNotification($info['message'], null, null, $extral)
		    ->setNotificationAlert($info['message'])
		    ->setOptions(null, null, null, $is_production)
		    ->send();
    ...
    public function pushToAll($info){
    	$extral = array();
    	foreach ($info as $ko => $vo){
    		if(substr($ko, 0, 2) != 't_') continue;
    		$extral['type'] = $ko;
    ...
    public function scheduleInfo($info){
    	$extral = array();
    	foreach ($info as $ko => $vo){
    		if(substr($ko, 0, 2) != 't_') continue;
    		$extral['type'] = $ko;
		...
		$obj = $this->push()
		    ...
		    ->setNotificationAlert($info['message'])
		    ->setOptions(null, null, null, $is_production);
		if (empty($info['alias'])) $obj->addAllAudience();
		else $obj->addAlias($info['alias']);
		$result = $obj->build();
		$trigger = array('time'=>date('Y-m-d H:i:s', $info['schedule_time']));
		$name = $info['schedule_name'];
		$enabled = $info['schedule_time'] > time() ? true : false;
		if (empty($info['schedule_id'])){
			return $this->schedule()->createSingleSchedule($name, $result, $trigger);
		} else {
			return $this->schedule()->updateSingleSchedule($info['schedule_id'], $name, $enabled, $result, $trigger);
		}
    }
    
    public function scheduleDelete($info){
    	if(empty($info['schedule_id'])) return false;
    	return $this->schedule()->deleteSchedule($info['schedule_id']);
    }
