...
class BackController extends FController {
	public function _initialize(){
		parent::_initialize();
		
		$set = array(
			'in'=>'aftersales',
			'ac'=>'aftersales_back_index',
		);
		$this->sbset($set);
    }
    ...
    public function nocheckAction() {
    	$this->sbset('aftersales_back_nocheck');
    	$map = array('status'=>0);
    	$this->listDisplay($map);
    }
    
    public function passAction() {
    	$this->sbset('aftersales_back_pass');
    	$map = array('status'=>1);
    	$this->listDisplay($map);
    }
    
    public function backingAction() {
    	$this->sbset('aftersales_back_backing');
    	$map = array('status'=>3);
    	$this->listDisplay($map);
    }
    
    public function backmoneyAction() {
    	$this->sbset('aftersales_back_backmoney');
    	$map = array('status'=>5);
    	$this->listDisplay($map);
    }
    
    public function finishAction() {
    	$this->sbset('aftersales_back_finish');
    	$map = array('status'=>6);
    	$this->listDisplay($map);
    }
    
    public function nopassAction() {
    	$this->sbset('aftersales_back_nopass');
