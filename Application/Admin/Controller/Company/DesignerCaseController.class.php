...
class DesignerCaseController extends FController {
	...
	public function addAction($designer_id=0){
		...
		if(IS_POST){
			...
			$params['distributor_id']=$design_info['distributor_id'];
			...
			$this->success('添加成功',U('Company/DesignerCase/index',array('designer_id'=>$designer_id)),0);
		}
		$this->display('edit');
	}
	...
	public function editAction($designer_id = 0){
		...
		if(IS_POST){
			...
			$this->success('修改成功',U('Company/DesignerCase/index',array('designer_id'=>$designer_id)),0);
		}
	...
	public function delAction($id = 0){
		...
		$this->success('删除成功',U('Company/DesignerCase/index',array('designer_id'=>$info['designer_id'])),0);
	}
	...
	public function listAction(){
    	...
    	if ($result['count'] > 0){
			//var_dump($result);die();
    		$this->assign('list', $result['list']);
			$this->assign('designers',$result['designers']);
    		$pager = new Pager($result['count'], $result['pagesize']);
    		$this->assign('pager', $pager->show());
    	}
		$this->assign('get', $get);
	...
	public function is_key_indexAction($id){
		try{
			$this->designerCaseService()->caseIsIndex($id);
		}catch(\Exception $e){
			$this->error($e->getMessage());
		}
		$this->success('推荐首页成功');
