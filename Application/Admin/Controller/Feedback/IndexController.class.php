...
class IndexController extends FController {
	...
	public function editAction($id = 0){
		...
		if(IS_POST){
			$params = I('post.');
			$params['log_id'] = $info['log_id'];
			$params['admin_id'] = session('uid');
			try {
				$result = $this->feedbackService()->remark($params);
			} catch (\Exception $e) {
				$this->error($e->getMessage());
			}
			$this->success('修改成功', session('bakc_url'), 0);
		}
		$this->assign('info', $info);
		$this->display();
	}
	...
	public function delAction($id = 0){
		...
		if(empty($info)) $this->error('内容不存在');
		try {
			$result = $this->feedbackService()->feedbackDelete($info['log_id']);
		} catch (\Exception $e) {
			$this->error($e->getMessage());
		}
		$this->success('删除成功', session('bakc_url'), 0);
	}
