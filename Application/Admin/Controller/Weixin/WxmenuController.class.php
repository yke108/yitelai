...
class WxmenuController extends FController {
    ...
    public function indexAction(){
    	...
    	$weixinaddress = array(
    			array(
    					'label'=>'商场首页',
    					'url'=>str_replace('gajadmin/', '', DK_DOMAIN_WAP.U('index/index'))
    			),
    			array(
    					'label'=>'专属名片',
    					'url'=>str_replace('gajadmin/', '', DK_DOMAIN_WAP.U('user/invite'))
    			),
    			array(
    					'label'=>'我的推荐',
    					'url'=>str_replace('gajadmin/', '', DK_DOMAIN_WAP.U('user/invite/recommend'))
    			),
    			array(
    					'label'=>'我的潜在客户',
    					'url'=>str_replace('gajadmin/', '', DK_DOMAIN_WAP.U('user/invite/list'))
    			),
    			array(
    					'label'=>'我的返利',
    					'url'=>str_replace('gajadmin/', '', DK_DOMAIN_WAP.U('user/invite/rebate'))
    			),
    			array(
    					'label'=>'我的订单',
    					'url'=>str_replace('gajadmin/', '', DK_DOMAIN_WAP.U('order/index'))
    			),
    			array(
    					'label'=>'个人中心',
    					'url'=>str_replace('gajadmin/', '', DK_DOMAIN_WAP.U('user/index'))
    			),
    			array(
    					'label'=>'退换货订单',
    					'url'=>str_replace('gajadmin/', '', DK_DOMAIN_WAP.U('order/index/finish'))
    			),
    ...
    public function editAction($menu_id = 0){
		...
		if(IS_POST){
			...
			$rules = array(
					...
					array('menu_type',array(0,1,2,3),'菜单类型的值不正确',2,'in'),
			);
