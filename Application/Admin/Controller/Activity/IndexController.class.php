...
class IndexController extends FController {
	...
	public function addAction(){
		if(IS_POST){
			$post = $_POST;
			try {
				$result = $this->activityInfoService()->infoCreateOrModify($post);
			} catch (\Exception $e) {
	...
	public function editAction($activity_id = 0){
		$info = $this->activityInfoService()->getInfo($activity_id);
	...
	public function delAction($activity_id = 0){
		try {
			$result = $this->activityInfoService()->infoDelete($activity_id);
	...
	public function recommendAction($activity_id = 0){
		try {
			$result = $this->activityInfoService()->infoRecommend($activity_id);
	...
	public function openAction($activity_id = 0){
		try {
			$result = $this->activityInfoService()->infoOpen($activity_id);
			$this->success('操作成功', session('back_url'));
		} catch (\Exception $e) {
			$this->error($e->getMessage());
		}
