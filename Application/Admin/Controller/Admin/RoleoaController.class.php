...
class RoleoaController extends FController {
	...
	public function addAction(){
		...
		if(IS_POST){
			...
			$params['org_id'] = $this->m_org_id;
			try {
				$result = $this->adminService()->oaRoleCreateOrModify($params);
			} catch (\Exception $e) {
	...
	public function delAction($id = 0){
		...
		try {
			...
			$result = $this->adminService()->oaRoleDelete($params);
		} catch (\Exception $e) {
			$this->error($e->getMessage());
		}
		$this->success('删除成功');
