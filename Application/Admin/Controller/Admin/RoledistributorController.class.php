...
class RoledistributorController extends FController {
	...
	public function _initialize(){
		...
		$set = array(
			'in'=>'admin',
			'ac'=>'admin_roledistributor_index',
		);
		$this->sbset($set);
	...
	public function addAction(){
		...
		if(IS_POST){
			$params = I('post.');
			$params['sys_id'] = $this->d_sys_id;
			$params['org_id'] = $this->d_org_id;
			try {
	...
	public function delAction($id = 0){
		...
		if(empty($info)) $this->error('内容不存在');
		//特定角色不可删除
		if (in_array($id, array(49,50,53,54,55,57,65,66))) $this->error('角色不可删除');
