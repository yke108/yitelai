<?php
namespace Distributor\Controller\Material;
...
class IndexController extends FController {
	...
	private function listDisplay($map = array()){
		...
		//设计师列表
		$params = array('distributor_id'=>$this->org_id);
		$designer_list = $this->designerService()->infoList($params);
	...
	public function delAction($material_id = 0){
		...
		try {
			$this->materialService()->delete($params);
		} catch (\Exception $e) {
			$this->error($e->getMessage());
		}
		$this->success('删除成功');
	...
	public function delallAction(){
		$params = array(
				'material_ids'=>I('material_ids'),
				'distributor_id'=>$this->org_id,
		);
		try {
			$this->materialService()->delall($params);
		} catch (\Exception $e) {
			$this->error($e->getMessage());
		}
	...
	public function upAction(){
		...
		try {
			$this->materialService()->up($params);
		} catch (\Exception $e) {
			$this->error($e->getMessage());
		}
		$this->success('操作成功', session('back_url'));
	...
	public function downAction(){
		...
		try {
			$this->materialService()->down($params);
		} catch (\Exception $e) {
			$this->error($e->getMessage());
		}
		$this->success('操作成功', session('back_url'));
	...
	public function recommendAction($material_id){
		...
		try {
			$this->materialService()->recommend($params);
		} catch (\Exception $e) {
			$this->error($e->getMessage());
		}
		$this->success('操作成功', session('back_url'));
	...
	public function showAction($material_id){
		...
		try {
			$this->materialService()->show($params);
		} catch (\Exception $e) {
			$this->error($e->getMessage());
		}
		$this->success('操作成功', session('back_url'));
