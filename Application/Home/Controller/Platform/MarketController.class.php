...
class MarketController extends BaseController {	
    ...
    public function indexAction() { /*招商经理*/
    	$get = I('get.');
    	$this->assign('get', $get);
    ...
    public function infoAction($user_id = 0) { /*NoPurview*/
    	...
    	$this->assign('page_title', '招商经理-详情');
    	$this->display();
    }
    
    public function brandAction($user_id = 0) { /*NoPurview*/
    	$info = $this->userService()->getUserInfo($user_id);
    	if (empty($info)) $this->error('用户不存在');
    	$this->assign('info', $info);
    ...
    public function districtAction($user_id = 0) { /*NoPurview*/
    	$info = $this->userService()->getUserInfo($user_id);
    	if (empty($info)) $this->error('用户不存在');
    	$this->assign('info', $info);
    ...
    public function storeAction($user_id = 0) { /*NoPurview*/
    	$info = $this->userService()->getUserInfo($user_id);
    	if (empty($info)) $this->error('用户不存在');
    	$this->assign('info', $info);
    	
    	//店铺
    	$map = array('status'=>Status::DistributorStatusNormal, 'status'=>Status::DistributorStatusNormal, 'distributor_id'=>array('in', $info['distributor_ids']));
    	$distributor_list = $this->distributorService()->getAllList($map);
    	$this->assign('distributor_list', $distributor_list);
